...
public enum FileLang {
  ...
  COMMAND_LIST_HEADER_SPLITTER("command_list_header_splitter", "&a&l------------------------------"),
  COMMAND_LIST_HEADER_GROUP("command_list_header_group", "&e&lGroup&7: &9{0}"),
  ...
  ERROR_CUSTOM_TAB_COMPLETE_NOT_REGISTERED("error_custom_tab_complete_not_registered", "The command ({0}) is not registered so a custom tab-complete handler could not be set for this command. Make sure it's the command registered to the server, not the alias."),
  ;
  ...
  public String getString(String[] variables) {
    return TabCompleteFilter.getInstance().getFileManager().getStringFromFile(File.LANG.name().toLowerCase(), getPath(), variables);
  }

  public TextComponent getTextComponent(String[] variables) {
    return new TextComponent(TabCompleteFilter.getInstance().getFileManager().getStringFromFile(File.LANG.name().toLowerCase(), getPath(), variables));
  }
}
